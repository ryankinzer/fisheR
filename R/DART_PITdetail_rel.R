#' @title DART PIT Tag Observations by Release Site Detail
#'
#' @description DART PIT Tag Observations by Release Site Detail
#'
#' @author Tyler Stright, Ryan Kinzer
#'
#' @param rel_site PTAGIS code for desired observation site. see fisheR::DART_sites()
...
#'
#' @source \url{https://www.cbr.washington.edu/dart/query/pit_rel_detail}
#'
...
#' @examples
#'
#' DART_PITdetail_rel(rel_site = 'IMNTRP', species = 'Chinook', run = 'All',
#'                    rear_type = 'All', start_date = '02/01/2020', end_date = '02/01/2021')

DART_PITdetail_rel = function(rel_site,
  ...
  if(is.null(rel_site))stop('You must provide a PTAGIS release site code (rel_site). see DART_sites("release")')
  stopifnot(!is.null(start_date)|!is.null(end_date))
  stopifnot(grepl('\\d{2}/\\d{2}/\\d{4}', start_date)|grepl('\\d{2}/\\d{2}/\\d{4}', end_date))

  # rel_site <- match.arg(rel_site, several.ok = FALSE)
  ...
  codes <- DART_codes(obs_site=NULL, species, run, rear_type, life_stage=NULL)
  ...
  queryList = list(sc = 1,
                   outputFormat = 'csv',
                   year = syear,
                   rel_site = rel_site,  # release site code (not verbose)
  ...
  # add spp_code to dat if missing
  if(!'spp_code' %in% names(dat)) { dat$spp_code <- codes[2] }
